...
- MySQL 仓储查询与写入：活动基础信息、活动账户总/月/日、活动 SKU 与次数配置、活动参与订单查询、活动参与下单事务、活动兑换订单查询、活动配送边界与成功事务、活动与策略库存写入、策略基础信息、策略奖品列表与明细、策略权重规则、策略锁规则、策略规则树装配、策略活动账户参与次数、奖品配置与待发奖任务、奖品发放事务、积分账户、积分支付与返利入账事务、返利配置与签到订单、返利记录写入、积分兑换订单重复映射、任务状态更新 sqlmock 覆盖
